...
contains
  ...
  subroutine pes_init(nwat)
    ...
    open(20,file='/insomnia001/home/hkt2112/Code/vstr/examples/potentials/coef_qAQUA-pol/coeff_diff_2b.dat',status='old')
    ncoef = size(coef2)
    do i=1,ncoef
       read (20,*) coef2(i)
    end do
    ...
    open(20,file='/insomnia001/home/hkt2112/Code/vstr/examples/potentials/coef_qAQUA-pol/coeff_diff_3b.dat',status='old')
    ncoef = size(coef3_2)
    do i=1,ncoef
       read (20,*) coef3_2(i)
    end do
    ...
    open(20,file='/insomnia001/home/hkt2112/Code/vstr/examples/potentials/coef_qAQUA-pol/coeff_diff_3b_lr_4th.dat',status='old')
    ncoef = size(coef3_1)
    do i=1,ncoef
       read (20,*) coef3_1(i)
    end do
    ...
    open(20,file='/insomnia001/home/hkt2112/Code/vstr/examples/potentials/coef_qAQUA-pol/coeff_grp200_diff',status='old')
    ncoef = size(coef4)
    do i=1,ncoef
       read (20,*) coef4(i)
    end do
  ...
  subroutine getpot(x,eng,mu)
    ...
    xttm = xttm*auang
    call potential(nw,xttm,gdttm,engttm,mu)
...
!  end subroutine bond
  ...
  subroutine hessian(x,H)
    ...
    do i=1,size(x)
      tx=x;tx(i)=tx(i)+eps;call pot_gd(tx,pot,gd1(:,i),mu)
      tx=x;tx(i)=tx(i)-eps;call pot_gd(tx,pot,gd2(:,i),mu)
    end do
  ...
  subroutine num_hessian(x,H)
    ...
    tx=x
    call getpot(tx,fx,mu)
    do i=1,size(x)
      do j=i+1,size(x)
        tx=x;tx(i)=tx(i)+eps;tx(j)=tx(j)+eps;call getpot(tx,f_ff,mu)
        tx=x;tx(i)=tx(i)+eps;tx(j)=tx(j)-eps;call getpot(tx,f_fb,mu)
        tx=x;tx(i)=tx(i)-eps;tx(j)=tx(j)+eps;call getpot(tx,f_bf,mu)
        tx=x;tx(i)=tx(i)-eps;tx(j)=tx(j)-eps;call getpot(tx,f_bb,mu)  
    ...
    do i=1,size(x)
       tx=x;tx(i)=tx(i)+eps;call getpot(tx,f_ff,mu)
       tx=x;tx(i)=tx(i)-eps;call getpot(tx,f_bb,mu)
