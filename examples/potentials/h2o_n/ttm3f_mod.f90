...
!   use ttm3f_mod
   ...
   subroutine ttm3f(Nw,RR,dRR,En,mu)
...
!   use ttm3f_mod
   ...
   do iw=1, Nw
      ...
      mu = mu + q3(1)*r1(1:3,1) + q3(2)*r1(1:3,2) + q3(3)*r1(1:3,3)
   enddo
   ...
   do iw=1,Nw
      mu(1) = mu(1) + dip(fM3 + (iw-1)*3)
      mu(2) = mu(2) + dip(fM3 + (iw-1)*3 + 1)
      mu(3) = mu(3) + dip(fM3 + (iw-1)*3 + 2)
   enddo
